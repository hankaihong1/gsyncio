# gsyncio 原语选择决策表（Primitive Selection Guide）
**[English](CHOOSING.md)**
...
| 跑 ASGI 服务（FastAPI 等） | `GsyncioASGIWorker` | 多事件循环 worker |
...
- **异常** — `ChannelClosedError`（通道已关闭）、`ThreadPoolClosedError`（池已关闭）、`TimeoutError`（gsyncio 自己的超时，非内置）、`WouldBlock`（非阻塞操作无数据）、基类 `GsyncioError`。
