"""Shared helpers for lazy-importing the optional ``_multiloop_core`` Rust extension."""
...
def _try_import_rust_class(module_path: str, class_name: str) -> Any | None:
    ...
    The ``_multiloop_core`` C-extension is optional: it is only compiled on
    supported platforms. Every module that depends on it follows the same
    pattern, so the fallback is centralized here.

    :param module_path: Full dotted module path (e.g. ``"multiloop._multiloop_core"``).
